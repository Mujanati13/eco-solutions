-- VPS Complete Wilaya Update SQL Script
-- This script updates your production database with the complete wilaya data and pricing

-- 1. Create backup table (IMPORTANT!)
CREATE TABLE wilayas_backup_production_final AS SELECT * FROM wilayas;

-- 2. Add pricing columns (ignore errors if they already exist)
-- Check and add home_delivery_price column
SET @sql = IF((SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'wilayas' AND COLUMN_NAME = 'home_delivery_price') = 0, 'ALTER TABLE wilayas ADD COLUMN home_delivery_price DECIMAL(8,2) DEFAULT 0', 'SELECT "home_delivery_price column already exists"');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

-- Check and add office_delivery_price column
SET @sql = IF((SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'wilayas' AND COLUMN_NAME = 'office_delivery_price') = 0, 'ALTER TABLE wilayas ADD COLUMN office_delivery_price DECIMAL(8,2) DEFAULT 0', 'SELECT "office_delivery_price column already exists"');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

-- Check and add pickup_delivery_price column
SET @sql = IF((SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'wilayas' AND COLUMN_NAME = 'pickup_delivery_price') = 0, 'ALTER TABLE wilayas ADD COLUMN pickup_delivery_price DECIMAL(8,2) DEFAULT 0', 'SELECT "pickup_delivery_price column already exists"');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

-- Check and add express_delivery_price column
SET @sql = IF((SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'wilayas' AND COLUMN_NAME = 'express_delivery_price') = 0, 'ALTER TABLE wilayas ADD COLUMN express_delivery_price DECIMAL(8,2) DEFAULT 0', 'SELECT "express_delivery_price column already exists"');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

-- 3. Update all wilaya records with complete data and pricing
...
-- DROP TABLE wilayas;
-- RENAME TABLE wilayas_backup_production_final TO wilayas;