...
const Dashboard = () => {
  ...
  // Remove old isAdmin declaration since it's now from usePermissions
  ...
  const handleFileUpload = async (file) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Show warnings if any (for missing fields that were still imported)
        ...
        // Show errors if any (for rows that failed to import)
        ...
        throw new Error(errorData.error || errorData.message || t('dashboard.importError'))
      }
    } catch (error) {
  ...
  const handleDistributeOrders = async () => {
    try {
      ...
      if (response.ok) {
        const result = await response.json()
        message.success(t('dashboard.distributeSuccess', { count: result.distributed }))
        fetchDashboardData()
      } else {
        const errorData = await response.json()
        throw new Error(errorData.message || t('dashboard.distributeError'))
      }
    } catch (error) {
            ...
            {orderDistribution.length > 0 ? (
              <Pie
                data={orderDistribution.map(item => ({
                  type: t(`orders.statusTypes.${item.status}`) || item.status,
                  value: item.count,
