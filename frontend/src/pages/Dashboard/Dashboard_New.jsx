  ...
  const handleFileUpload = async (file) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        message.error(errorData.error || t('dashboard.importFailed'))
      }
      
    } catch (error) {
      console.error('Import error:', error)
      message.error(t('dashboard.importFailed'))
    } finally {
      setUploading(false)
      setUploadProgress(0)
    }
  ...
  const handleDistributeOrders = async () => {
    try {
      ...
      if (response.ok) {
        ...
        const errorData = await response.json()
        message.error(errorData.error || t('dashboard.distributeFailed'))
      }
    } catch (error) {
      console.error('Distribute error:', error)
      message.error(t('dashboard.distributeFailed'))
    }
  }
